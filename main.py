...
class HexoToTypecho():
    def __init__(self, d, host, user, database, files, passwd=''):
        self.host = host
        self.user = user
        self.passwd = passwd
        self.port = 3306
    ...
    def parse_hexo_md(self, d, file):
        ...
        if not date:
            date = "-".join(file.split("-")[:3]) + " 00:00:00"
        else:
            date = date.group(1)
        ...
        try:
            if not re.search(r'category : [ ]*(.*?)\n', s).group(1):
                if re.search(r'category : [ ]*\n(.*?)\n---', s, re.S):
                    items = re.search(r'category : [ ]*\n(.*?)\n---', s, re.S).group(1)
                    categories = re.findall(r'- (.*?)\n', items)
                else:
                    categories = ''
            else:
                categories = re.search(r'category : [ ]*(.*?)\n', s).group(1)
        except AttributeError as e:
            print(e)
            categories = ''
        # 正文提取
    ...
    def insert_post(self, d, file):
        data = self.parse_hexo_md(d, file)
        self.data = data
        db = self.db
        cur = self.cur
        modified = int(time.mktime(time.localtime(os.stat(d + file).st_mtime)))
        sql = '''
...
if __name__ == '__main__':
    ...
    print('总有', len(files))
    HexoToTypecho(d="first/", host='123.206.230.129', user='root', database='blog', files=files, passwd='COordinate54188')
